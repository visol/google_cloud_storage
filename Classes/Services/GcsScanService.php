...
class GcsScanService
{
    ...
    {
        $this->getGcsResourceService()->deleteAll();
        $this->getGcsFolderService()->deleteAll();
    }
    ...
    public function scan(): array
    {
        ...
        $cloudinaryFolder = $this->getGcsPathService()->computeGcsFolderPath(DIRECTORY_SEPARATOR);
        ...
        do {
            ...
            $this->log(
                '[API][SEARCH] Gcs\Search() - fetch resources from folder "%s" %s',
                [
                    $cloudinaryFolder,
            ...
            /** @var Search $search */
            $search = new \GoogleCloudStorage\Search();
            ...
            if (is_array($response['resources'])) {
                foreach ($response['resources'] as $resource) {
                    ...
                    $fileIdentifier = $this->getGcsPathService()->computeFileIdentifier($resource);
                    if ($this->io) {
                        $this->io->writeln($fileIdentifier);
                    }

                    // Save mirrored file
                    $result = $this->getGcsResourceService()->save($resource);
    ...
    {
        $this->getGcsResourceService()->markAsMissing();
        $this->getGcsFolderService()->markAsMissing();
    }
    ...
    protected function postScan(): void
    {
        ...
        $this->statistics[self::DELETED] = $this->getGcsResourceService()->deleteAll($identifier);
        $this->statistics[self::FOLDER_DELETED] = $this->getGcsFolderService()->deleteAll($identifier);
    }
    ...
    /**
     * @return object|GcsResourceService
     */
    protected function getGcsResourceService(): GcsResourceService
    {
        return GeneralUtility::makeInstance(GcsResourceService::class, $this->storage);
    }

    /**
     * @return object|GcsFolderService
     */
    protected function getGcsFolderService(): GcsFolderService
    {
        return GeneralUtility::makeInstance(GcsFolderService::class, $this->storage);
    }

    /**
     * @return GcsPathService
     */
    protected function getGcsPathService(): GcsPathService
    {
        if (!$this->gcsPathService) {
            $this->gcsPathService = GeneralUtility::makeInstance(
                GcsPathService::class,
                $this->storage
            );
        }

        return $this->gcsPathService;
    }
