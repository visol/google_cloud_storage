...
use Visol\GoogleCloudStorage\Domain\Repository\ResponsiveBreakpointsRepository;
...
class GcsImageService
{
    ...
    public function getResponsiveBreakpointData(File $file, array $options): array
    {
        ...
        if (!$responsiveBreakpoints) {
            $this->initializeApi();
            $response = \GoogleCloudStorage\Uploader::explicit($publicId, $options);
    ...
    {
        /** @var GcsUploadService $cloudinaryUploadService */
        $cloudinaryUploadService = GeneralUtility::makeInstance(GcsUploadService::class);
        return $cloudinaryUploadService->uploadLocalFile('');
    }

    /**
     * @return object|GcsPathService
     */
